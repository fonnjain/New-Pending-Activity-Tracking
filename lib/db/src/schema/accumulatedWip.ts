...
// Permanent per-(project, structure) accumulated WIP throughput totals.
// Additive, display-only overlay -- never feeds parsing / activity / dedup /
// ageing / milestone / dispatch math. Rebuilt deterministically by a full
// id-ASC replay of the WIP import history (mirrors the order-dispatch
// engine), from the very first WIP file (no cutoff -- these are lifetime
// throughput counters, not a point-in-time balance).
//
// The underlying replay is inherently MARK-wise (every accumulation event is
// booked against one identity/mark); this table is the STRUCTURE-wise
// rollup of that mark-wise ledger, and the API further rolls structures up
// into project-wise totals -- mark -> structure -> project, the same
// hierarchy the Finished Good WIP Computed figure consumes.
...
export const accumulatedWipTable = pgTable(
  "accumulated_wip",
  {
...
// dispatch_ledger). Mark-wise (one row per identity per event); `structure`
// lets any future drill-down roll the same events up to the structure/project
// levels without re-joining the import history. Not currently exposed via the
// API; kept for future drill-down / debugging.
