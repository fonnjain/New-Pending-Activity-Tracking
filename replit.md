...
- **Turnaround** (`turnaround.tsx`, `/turnaround`) — 8-state deep-dive: TurnaroundWarnings ("Turnaround Lifecycle" card) then, immediately after it, a tabbed Projects/Contractors/Stages breakdown (`TurnaroundBreakdown`, mirrors the Stuck Projects tab layout but driven by lifecycle/overrun metrics, not velocity: a project leaderboard by breach score with mark drill, and contractor/stage overrun tables — no summary tiles, the lifecycle card already shows On track/Pre-warning/Breached + the full 8-state strip) + urgency worklist (by daysToTarget) + the AI turnaround report (`components/ai-turnaround-report.tsx`).
