...
- **Overview** (`overview.tsx`) — snapshot hub: headline KPIs + Changes panel + two compact linking cards (Turnaround snapshot, Velocity snapshot). Stalled and slow are counted on the same filter-scoped identity basis as the Stuck page.
- **Turnaround** (`turnaround.tsx`, `/turnaround`) — 8-state deep-dive: TurnaroundWarnings + a tabbed Projects/Contractors/Stages breakdown (`TurnaroundBreakdown`, mirrors the Stuck Projects tab layout but driven by lifecycle/overrun metrics, not velocity: summary tiles Breached/Pre-warning/On track/N/A, a project leaderboard by breach score with mark drill, and contractor/stage overrun tables) + urgency worklist (by daysToTarget) + the AI turnaround report (`components/ai-turnaround-report.tsx`).
- **Stuck Projects** (`stuck-projects.tsx`, `/stuck`) — project leaderboard by stuck score (stalled+slow weight share) with mark drill (pace/ETA/eta_gap/trend/days-since-movement), contractor + stage views.
