...
- Reads `Sheet1` (else first sheet). Columns are read by **header name, not position**. Original format: 21 cols (col A = Project Code … col S = Last Production Entry Date, col T = Work Order No., col U = WO Batch No.). Newer export format (≥ Jul 2026): 24 cols — prepends "Type" (col A), inserts "Job Card Date" (col F) + "Job Card Status" (col G), renames "WO Batch No." → "Batch No." (now col X). Parser accepts both header names; extra columns are silently ignored.
- **MFC batch (additive display grouping).** `mfcBatch` = trim/upper of "WO Batch No." OR "Batch No." (whichever is present); blank → "Z" (sorts last). `workOrderNo` = "Work Order No." (emptyToNull, stored only). Both stored on `record_pool`. The RAW pre-"Z" batch + `workOrderNo` are hashed; the "Z" substitution and trim/upper normalization are NOT. For TLT, MFC is a grouping level between Project and Structure (Project→MFC→Structure→Mark), per-project; global + drilldown MFC filters and MFC rollups. NTLT unaffected (stays Section-grouped).
