...
- **Engine (single source of truth): `@workspace/domain` (`lib/domain/src/index.ts`).** `TurnaroundSettings`, `GraceMode` (`absolute`|`percent`), `AlertStatus`, `AlertResult{status,target,overrun}`, `DEFAULT_TURNAROUND_SETTINGS`, `cumulativeTargets`, `cumulativeTarget`, `alertStatus`. Reuses the SAME `isKnownActivity`/`normalizeActivity` as the ordering layer (case-insensitive). `alertStatus` returns **`na`** whenever the target is null (activity outside `PROCESS_SEQUENCE`) OR `ageingDays` is null (blank production date) — never a false green/red.
- **Grace bands.** `yellowMax`/`orangeMax` are the bands BEYOND the cumulative target. In `absolute` mode they are extra days; in `percent` mode they are a percentage of that activity's cumulative target. **Per-activity overrides** (`overrides[code] = {yellowMax,orangeMax}`) replace the global bands for that code only.
- **Persistence.** Singleton `settings` table (`lib/db/src/schema/settings.ts`, `SETTINGS_SINGLETON_ID`). `GET /settings` returns the stored row or `DEFAULT_TURNAROUND_SETTINGS`; `PUT /settings` upserts. **`PUT /settings` validates bands server-side** (non-negative, `yellowMax <= orangeMax`, for the global bands AND every override) and **echoes back the SAME normalized (rounded) object it persists** so the client cache never drifts from stored values. Route: `artifacts/api-server/src/routes/settings.ts`. **Note: the `settings` table must exist in prod too — run `pnpm --filter @workspace/db run push` against prod after deploying.**
- **Frontend.** Settings provider in `artifacts/tracker/src/lib/settings.tsx` (loads via `useGetSettings`, applies a local draft live, debounced `PUT` persist, reset); helpers (`ALERT_LABELS`, `statusTextColor`, `statusBgColor`) in `lib/turnaround.ts`. Overview (`pages/overview.tsx`) gets a collapsible settings panel (`turnaround-settings.tsx`: ideal-days grid with cumulative targets, global yellow/orange bands + grace-mode toggle, per-activity override table, reset) and a warnings block (`turnaround-warnings.tsx`: summary strip of Green/Yellow/Orange/Red/na counts, per-activity breakdown bars, Orange/Red drill table) — all respecting the active filters. Status colors are applied in the Ageing + Activity tables (via `components/status-dot.tsx`, alongside the fixed ageing scale which is kept). Reports (`pages/reports.tsx`) + Excel export add `cumulativeTarget`/`overrun`/`alertStatus` columns. New ageing-orange token (`--ageing-orange`, hsl 24 95% 53%) added to `index.css`.
