...
- **Turnaround** (`turnaround.tsx`, `/turnaround`) — 8-state deep-dive: a tabbed Projects/Contractors/Stages breakdown FIRST (`TurnaroundBreakdown`, at the very top — mirrors the Stuck Projects tab layout but driven by lifecycle/overrun metrics, not velocity: a project leaderboard by breach score with mark drill, and contractor/stage overrun tables — no summary tiles), then the TurnaroundWarnings "Turnaround Lifecycle" card (On track/Pre-warning/Breached + the full 8-state strip + by-activity bars) + urgency worklist (by daysToTarget) + the AI turnaround report (`components/ai-turnaround-report.tsx`).
