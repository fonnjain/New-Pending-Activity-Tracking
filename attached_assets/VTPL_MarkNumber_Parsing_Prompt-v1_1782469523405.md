...
### Rule 0 — Normalize the malformed leading dash (run before all other rules)
Some source rows carry a stray leading `-` on the alias, in **both** Col G and Col H. Normalize both:
- If **Col G** starts with a single leading `-`, remove that one leading dash (`-069-2NBE1` → `069-2NBE1`).
- In **Col H**, if the alias portion right after the project starts with `-` (i.e. H matches `<A> -...`), remove that one dash too (`946 -069-2NBE1-06` → `946 069-2NBE1-06`).

Remove **only** this leading dash — dashes inside the value (e.g. `2DF-5`, `2HCT-1`, `4T1-P`, and the internal dashes of `069-2NBE1`) are left untouched. After this, the row parses by the normal rules below.
- Example: `A=946`, `G=-069-2NBE1`, `H=946 -069-2NBE1-06` → normalized `G=069-2NBE1`, `H=946 069-2NBE1-06` → Rule C gives structure `069-2NBE1`, mNo `06`, markNumber `946 \ 069-2NBE1 \ 06` (3-part, no ProMno).
- Verified: touches exactly 54 rows (all project 946), nothing else; all 54 parse cleanly.
