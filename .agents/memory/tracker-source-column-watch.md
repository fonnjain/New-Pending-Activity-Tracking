...
**How to apply:**
- Watched list = `WATCHED_SOURCE_COLUMNS` in api-server `lib/parse.ts`; declare a `mode` with each entry: `distribution`, `numeric`, or `coverage`. The panel renders the snapshot without a separate UI rule.
