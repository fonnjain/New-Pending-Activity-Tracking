...
- **"In-House" is a VIRTUAL filter group = stored category CNC OR SUB_CONTRACTOR — never a stored tag** (the 4-value enum is unchanged). It is a filter-only sentinel resolved at read time via one shared domain helper, so client/server filtering stays identical.
  **How to apply:** every contractor dropdown must build options + apply the predicate through the ONE shared frontend helper (grouped options builder + selection-match predicate). Do NOT hand-roll per-page contractor option lists — there are at least two dropdowns (global FilterBar + the Project-Wise drill-in) and they drift otherwise. Dropdown order is fixed: individual contractors → In-House (+CNC/Sub) → Out Vendors → Everything else (Unclassified).
