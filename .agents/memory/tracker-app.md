...
- **Commit must be idempotent against duplicate requests.** A large commit can take ~10s; the deployment proxy retries slow upstreams, so the SAME stagingId hits commit 2-3x. The first deletes-the-staging-row pattern made retries return `404 "Staged upload not found"` even though the import succeeded — a misleading "Import failed" toast for a working import.
  **Why:** the proxy/timeout retry of a slow commit is the real-world trigger (single-user tool, button disabled while pending — so not double-clicks).
  **How to apply:** do NOT delete the staging row on commit; mark it (`committed_import_id`) and replay the existing import (200) on duplicates; clean up via TTL. For true concurrency, claim atomically with `UPDATE ... WHERE committed_import_id IS NULL`; the loser must resolve the winner FIRST and only then delete its own orphan import (never return a just-deleted import). Reconstruct the response `ChangeSet` from the stored `changeSummary` (item arrays empty is fine for a replay).
...
- **Two different xlsx writers, on purpose.** The cleaned-file round-trip uses the free `xlsx` (SheetJS) and MUST stay plain (exact parse.ts layout, no styling). The user-facing "Export Excel" report uses `exceljs` instead, because the community `xlsx` build cannot write cell styles (bold/fills/borders/freeze/autofilter). Keep styled report exports on exceljs; never style the cleaned round-trip file.
  **Why:** user asked for a professionally formatted report (bold header, frozen row, auto-filter, auto-sized cols, totals row) — impossible with free SheetJS. **How to apply:** both writers are dynamically imported so neither bloats the initial bundle; report styling lives in `exportToXlsx` (export.ts), driven by an `XlsxColumn[]` spec (numeric/decimals/total flags).
